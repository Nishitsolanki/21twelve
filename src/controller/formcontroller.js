...
const createuser = async function (req, res) {
    ...
    try {
        ...
        if(!userRegistration) return res.status(400).send({sttaus:false,msg:"userregistration is required"})
        if(!isValid(userRegistration)) return res.status(400).send({sttaus:false,msg:"only contain aplhabets"})

        if(!username) return res.status(400).send({sttaus:false,msg:"username is required"})
        ...
        if(!taskname) return res.status(400).send({staus:false,msg:"taskname is required"})
        if(!isValid(taskname)) return res.status(400).send({status:false,msg:"taskname is only contain alphabet"})

        if(!taskdescription) return res.status(400).send({sttaus:false,msg:"taskdescription is required"})
        if(!isValid(taskdescription)) return res.status(400).send({status:false,msg:"taskdescription is only contain alphabet"})
...
const userlogin = async function(req,res){
    ...
    if(!userRegistration) return res.status(400).send({status:false,msg:"userregistration is required"})
    if(!isValid(userRegistration)) return res.status(400).send({status:false,msg:"userregistration is only contain alphabet"})

    if(!username) return res.status(400).send({status:false,message:"username is required"})
    if(!isValid(username))  return res.status(400).send({status:false,message:"username is only contain alphabe"})
    ...
    let token = jwt.sign({ userId: exist._id }, "Secretkeyof21twelve", { expiresIn: "24hr" })
...
const updateuser = async function(req,res){
    ...
    try {
     ...
     if (userRegistration) {
        if (!isValid(userRegistration)) {
           return res.status(400).send({ status: false, message: "userregestration is invalid is invalid!" })
        }

        if (!isValidName(userRegistration)) {
           return res.status(400).send({ status: false, message: "userRegestration should n't contain  any extraa charcter!" })
        }
     ...
     if (taskname) {
        if (!isValid(taskname)) {
           return res.status(400).send({ status: false, message: "taskname is invalid!" })
        }

        updates.taskname = taskname
     }

     let updateuser = await formmodel.findOneAndUpdate({ _id: userId }, { $set: updates }, { new: true })
 ...
 let deleteuser = async function (req, res) {
    try {
       ...
       let deleteu = await formmodel.findOneAndUpdate({ _id: userId }, { $set: { isDeleted: true } }, { new: true })
